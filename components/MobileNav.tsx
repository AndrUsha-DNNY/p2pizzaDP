...
const MobileNav: React.FC<MobileNavProps> = ({ currentView, onNavigate, hasUser }) => {
  const tabs = [
    ...
    { id: 'favorites', icon: Heart, label: 'Улюблене' },
    { id: 'history', icon: History, label: 'Історія', auth: true },
  ];

  return (
    <nav className="lg:hidden fixed bottom-0 left-0 right-0 bg-white/90 backdrop-blur-xl border-t border-orange-100 z-[100] pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.08)]">
        ...
        {tabs.map((tab) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[9px] font-black uppercase tracking-tight whitespace-nowrap">{tab.label}</span>
              {isActive && (
                <span className="absolute -top-1 w-1 h-1 bg-orange-500 rounded-full animate-pulse" />
              )}
            </button>
          );
        })}
