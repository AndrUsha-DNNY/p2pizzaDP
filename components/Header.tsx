...
const Header: React.FC<HeaderProps> = ({ user, onOpenAuth, onNavigate, cartCount, onOpenCart }) => {
  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-md border-b border-orange-100">
      <div className="container mx-auto px-4 py-3 flex items-center justify-between">
        <div 
          className="flex items-center gap-3 cursor-pointer group" 
          onClick={() => onNavigate('home')}
        >
          <div className="w-12 h-12 bg-orange-500 rounded-xl flex items-center justify-center text-white font-bold text-xl overflow-hidden shadow-lg group-hover:scale-105 transition-transform">
             <img 
               src={logo} 
               alt="P2Pizza Logo" 
        ...
        <nav className="hidden lg:flex items-center gap-6">
          ...
          <button onClick={() => onNavigate('box')} className="flex items-center gap-1 text-black hover:text-orange-600 font-bold transition-colors text-sm">
            <Package size={16} className="text-orange-500" /> BOX-меню
          </button>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          <a href={`tel:${phone}`} className="hidden md:flex items-center gap-2 bg-orange-50 px-3 py-2 rounded-xl text-orange-600 font-black text-xs hover:bg-orange-100 transition-colors">
            <Phone size={14} /> {phone}
          </a>
          ...
          {user?.role === 'admin' && (
            <button 
              ...
              className="p-2 hover:bg-orange-100 rounded-full transition-colors text-black"
          ...
          <button 
            onClick={onOpenAuth}
            className="flex items-center gap-2 hover:bg-orange-50 rounded-lg px-2 sm:px-3 py-2 transition-colors border border-transparent hover:border-orange-200"
          >
