...
import { Plus, Edit2, Trash2, X, Check, Key, Shield, Image as ImageIcon, Upload, List, ShoppingBag, Clock, CheckCircle, AlertCircle, MapPin, Sparkles, MessageSquare, Phone, Settings } from 'lucide-react';
import { saveAdminPassword, saveLogo, getStoredLogo, getStoredSpecial, saveSpecial, getStoredShopPhone, saveShopPhone } from '../store';
...
const AdminPanel: React.FC<AdminPanelProps> = ({ pizzas, onUpdatePizzas, orders, onUpdateOrderStatus, onClose }) => {
  ...
  const handleSaveSiteSettings = () => {
    ...
    setStatusMessage({ text: 'Налаштування сайту оновлено!', type: 'success' });
    setTimeout(() => setStatusMessage(null), 2000);
  };

  const handlePasswordChange = () => {
    if (newPass.length < 4) {
      setStatusMessage({ text: 'Пароль занадто короткий (мін. 4 символи)', type: 'error' });
      return;
    }
    saveAdminPassword(newPass);
    setStatusMessage({ text: 'Пароль успішно змінено!', type: 'success' });
    setNewPass('');
    setTimeout(() => {
  ...
  const handleLogoUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      reader.onloadend = () => {
        ...
        setStatusMessage({ text: 'Логотип успішно оновлено!', type: 'success' });
        setTimeout(() => setStatusMessage(null), 2000);
      };
                   ...
                   <div>
                     ...
                     <div className="aspect-video bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 overflow-hidden relative group">
                       ...
                       <div className="absolute inset-0 flex flex-col items-center justify-center p-4 text-center">
                          <span className="bg-orange-500 text-white px-3 py-1 rounded-full text-[8px] font-black uppercase mb-2">{siteSpecial.badge}</span>
                          <h3 className="text-white text-xl font-black mb-1 leading-tight">{siteSpecial.title}</h3>
                          <p className="text-white/80 text-[10px] line-clamp-2">{siteSpecial.description}</p>
                       </div>
                       <button onClick={() => specialImageInputRef.current?.click()} className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col items-center justify-center text-white gap-2 font-bold"><Upload size={32} />Завантажити нове</button>
                      ...
                      <tr key={order.id} className="border-b last:border-0 hover:bg-orange-50/20 transition-colors">
                        ...
                        <td className="p-5 text-right">
                            ...
                            >
                                ...
                                <option value="preparing">Готується (Start 8m timer)</option>
