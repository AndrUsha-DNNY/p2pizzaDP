...
import { 
  ...
  Palette, Phone, ShoppingBag, List, Copy, Upload, Camera, Image as ImageIcon
} from 'lucide-react';
import { 
    ...
    <div className="fixed inset-0 z-[150] bg-white overflow-y-auto text-black pb-24">
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="flex gap-4 mb-10 overflow-x-auto pb-2 scrollbar-hide">
          ...
          <button onClick={()=>setActiveTab('database')} className={`px-6 py-3 rounded-2xl font-black text-xs uppercase flex-shrink-0 transition-all ${activeTab==='database'?'bg-green-600 text-white shadow-lg':'bg-gray-100'}`}>База</button>
        </div>
        ...
        {activeTab === 'database' && (
          <div className="max-w-3xl space-y-6">
            ...
            <button onClick={handleSaveAll} className="w-full bg-green-600 text-white py-5 rounded-2xl font-black uppercase shadow-xl">Підключити та Синхронізувати</button>
          </div>
        )}
        ...
        {activeTab === 'settings' && (
          <div className="max-w-3xl space-y-8">
            <div className="bg-white p-8 border-2 border-orange-200 rounded-[2rem] shadow-sm">
              <h2 className="text-xl font-black uppercase mb-6 flex items-center gap-2"><ImageIcon size={20} className="text-orange-500"/> Брендинг</h2>
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-grow">
                    <p className="text-[10px] font-black uppercase text-gray-400 mb-2">Логотип сайту</p>
                    <button onClick={() => fileInputRef.current?.click()} className="bg-black text-white px-4 py-2 rounded-xl text-[10px] font-black uppercase flex items-center gap-2">
                      <Upload size={14}/> Завантажити файл
                    </button>
                    <input type="file" ref={fileInputRef} className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, 'logo')} />
                  </div>
                </div>
                <input className="w-full p-4 border rounded-xl bg-gray-50 font-medium text-xs" placeholder="Або вставте URL" value={siteLogo} onChange={e=>setSiteLogo(e.target.value)} />
              </div>
            </div>
            ...
            <div className="bg-white p-8 border-2 border-gray-100 rounded-[2rem] shadow-sm">
              <h2 className="text-xl font-black uppercase mb-6 flex items-center gap-2"><Phone size={20} className="text-green-500"/> Телефон закладу</h2>
            ...
            <button onClick={handleSaveAll} className="w-full bg-orange-500 text-white py-5 rounded-2xl font-black uppercase shadow-xl hover:bg-black transition-all">Зберегти всі налаштування</button>
          </div>
        )}

        {activeTab === 'menu' && (
          <div className="space-y-4">
            <button onClick={() => {setEditingId('new'); setEditForm({name: '', price: 0, category: 'pizza', image: ''})}} className="bg-green-500 text-white px-8 py-3 rounded-xl font-black uppercase text-xs mb-4 shadow-lg shadow-green-100">+ Додати нову позицію</button>
                    ...
                    <tr key={p.id} className="border-b hover:bg-orange-50/50 transition-colors">
                      ...
                      <td className="p-4 text-right">
                        <div className="flex gap-2 justify-end">
                          <button onClick={()=>{setEditingId(p.id); setEditForm(p);}} className="p-2 bg-blue-500 text-white rounded-lg shadow-md"><Edit2 size={14}/></button>
                          <button onClick={()=>{if(confirm('Видалити цей товар?')) onUpdatePizzas(pizzas.filter(x=>x.id!==p.id))}} className="p-2 bg-red-500 text-white rounded-lg shadow-md"><Trash2 size={14}/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        {activeTab === 'orders' && (
          <div className="space-y-4">
            {orders.length === 0 ? <p className="text-center py-20 text-gray-400 font-bold uppercase text-[10px] tracking-widest">Немає активних замовлень</p> : 
              orders.map(o => (
                <div key={o.id} className="bg-white p-6 rounded-3xl border border-gray-100 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 shadow-sm hover:shadow-md transition-shadow">
                  <div>
                    ...
                    <p className="text-orange-500 font-black text-[10px] uppercase mt-1">📞 {o.phone || 'Немає номера'}</p>
                    {o.address && <p className="text-gray-500 font-bold text-[10px] uppercase mt-1">🏠 {o.address}, буд. {o.houseNumber}</p>}
                  </div>
                  <div className="flex items-center gap-4 w-full md:w-auto justify-between md:justify-end">
                    <p className="font-black text-xl">{o.total} грн</p>
                    <select value={o.status} onChange={e=>onUpdateOrderStatus(o.id, e.target.value as OrderStatus)} className="p-2 border-2 rounded-xl font-black text-[10px] uppercase outline-none focus:border-orange-500 transition-colors bg-white">
          ...
          <div className="bg-white p-8 rounded-[3rem] w-full max-w-md shadow-2xl animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-4 mb-8">
              <div className="flex items-center gap-4 mb-4">
                <div className="w-24 h-24 bg-gray-50 rounded-2xl overflow-hidden border-2 border-dashed border-gray-200 flex items-center justify-center group relative cursor-pointer" onClick={() => pizzaImageRef.current?.click()}>
                ...
                <div className="flex-grow space-y-2">
                  <p className="text-[10px] font-black uppercase text-gray-400">Фото піци</p>
                  <button onClick={() => pizzaImageRef.current?.click()} className="text-[10px] font-black uppercase text-orange-500 hover:underline">Вибрати файл</button>
                  <input type="file" ref={pizzaImageRef} className="hidden" accept="image/*" onChange={(e) => handleFileUpload(e, 'pizza')} />
                </div>
              </div>
              <input className="w-full p-4 border rounded-xl bg-gray-50 font-bold focus:bg-white focus:border-orange-500 outline-none transition-all" placeholder="Назва страви" value={editForm.name} onChange={e=>setEditForm({...editForm, name: e.target.value})}/>
              <input type="number" className="w-full p-4 border rounded-xl bg-gray-50 font-black focus:bg-white focus:border-orange-500 outline-none transition-all" placeholder="Ціна (грн)" value={editForm.price} onChange={e=>setEditForm({...editForm, price: Number(e.target.value)})}/>
              <textarea className="w-full p-4 border rounded-xl bg-gray-50 font-medium text-xs h-24 focus:bg-white focus:border-orange-500 outline-none transition-all resize-none" placeholder="Опис інгредієнтів" value={editForm.description} onChange={e=>setEditForm({...editForm, description: e.target.value})}/>
            </div>
            <div className="flex gap-4">
              <button onClick={() => {
                ...
                setEditingId(null);
              }} className="flex-grow bg-orange-500 text-white py-4 rounded-2xl font-black uppercase text-xs shadow-lg shadow-orange-100 hover:bg-black transition-all">Зберегти</button>
              <button onClick={()=>setEditingId(null)} className="px-6 py-4 bg-gray-100 rounded-2xl font-black uppercase text-xs hover:bg-gray-200 transition-all">Скасувати</button>
            </div>
          </div>
        </div>
      )}
      ...
      {statusMessage && (
        <div className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-black text-white px-10 py-4 rounded-full font-black text-xs uppercase z-[300] shadow-2xl animate-in fade-in slide-in-from-bottom-10">
          {statusMessage}
        </div>
      )}
    </div>
